...
(function () {
  ...
  function 화자나누기(t) {
    ...
    return 조각.map(function (x) {
      ...
      if (!m) return '<p class="say"><span class="who"></span><span class="bt">' + 본문(x.trim()) + '</span></p>';
      ...
      return '<p class="say"><span class="who" title="' + esc(m[1]) + '">' +
             esc(이름) + '</span><span class="bt">' + 본문(m[2].trim()) + '</span></p>';
    }).join("");
  }
  ...
  function 보기나누기(t) {
    ...
    return '<div class="boki">' + 조각.map(function (x) {
      ...
      /* ★ p 가 flex 라 본문 안의 (가)·㉠ 표지가 제각각 flex 칸이 되어 세로로 늘어났다(2026-09-19 대표님 폰 캡처) — 본문을 한 칸으로 감싼다 */
      return '<p><span class="bk">' + esc(m[1]) + '</span><span class="bt">' + 본문(m[2].trim()) + '</span></p>';
    }).join("") + '</div>';
  }
  ...
  function 문항HTML(d, q, opt) {
    ...
    /* ★ 자료를 글로 보일지 그림으로 보일지 **먼저** 정한다.
       ...
       뒤섞인 글이 오히려 헷갈리게 한다. */
    var 글만으로되나 = 텍스트로되나(d, {});
    /* ★ gf = 사람이 읽고 「글만으로는 못 푼다」 고 판정한 문항(그림·지도·순서도가 글로 풀리며 A~C·갑을이 사라짐).
         글 거름(텍스트로되나)은 이런 것을 못 잡으므로, gf 면 자료 그림을 무조건 보인다(2026-09-19 문항 전수 읽기) */
    var 그림쓴다 = !!(q.fig && (!글만으로되나 || q.gf));
    ...
    if (보기 && !(그림쓴다 && q.gf)) 몸 += 보기나누기(보기);   /* gf 그림은 보기 상자까지 담고 있다 — 글 보기를 또 내면 두 번 나온다 */
  ...
  function 선지단추(d, opt) {
    ...
    var 열 = 선지.length && 선지.every(function (o) {
      return String(o).length <= 5 && !/[㉠-㉭ⓐ-ⓩ]/.test(String(o));       // ㉠ 은 상자로 그려져 넓다 — 「ㄱ, ㄴ」 처럼 아주 짧을 때만 가로로 — 14자까지 가로로 두니 폰(375px)에서 다섯 칸이 화면 밖으로 넘쳤다(2026-09-19 전수 실측 60여 문항)
    }) ? " row" : "";
